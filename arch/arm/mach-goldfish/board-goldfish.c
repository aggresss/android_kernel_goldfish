...
void goldfish_mask_irq(struct irq_data *irq_data)
{
	writel(irq_data->irq, IO_ADDRESS(GOLDFISH_INTERRUPT_BASE) +
					 GOLDFISH_INTERRUPT_DISABLE);
}

void goldfish_unmask_irq(struct irq_data *irq_data)
{
	writel(irq_data->irq, IO_ADDRESS(GOLDFISH_INTERRUPT_BASE) + GOLDFISH_INTERRUPT_ENABLE);
}
